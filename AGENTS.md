...
| **The devshell's `repl` command is BROKEN in this repository** — it invokes `nix repl --file` on a ci/repl.nix that gen-harness does not have. Consumers that ship one are unaffected; here the command exists and fails                                                                                             | `flakeModule.nix:465-468` declares it unconditionally, off `$FLAKE_ROOT`. Nothing in the tracked tree answers that path — `ls ci/repl.nix` ⇒ rc 2. Reported, not repaired: the fix is a design call about whether the command should be conditional                                                                                  |
